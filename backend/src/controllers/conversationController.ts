...
export const getConversationById = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    const conversation = await prisma.conversation.findFirst({
      where: { id: conversationId, members: { some: { id: req.user!.id } } },
      include: {
        members: {
          select: {
            id: true,
    ...
    res.json({ success: true, data: { conversation: formattedConversation } });
  }
);
...
export const createDirectConversation = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    const conversation = await prisma.conversation.create({
      data: {
        isGroup: false,
        members: { connect: [{ id: req.user!.id }, { id: userId }] },
      },
      include: {
        members: {
          select: {
...
export const addGroupMembers = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    await prisma.conversation.update({
      ...
      data: {
        members: { connect: newUserIds.map((id: string) => ({ id })) },
        updatedAt: new Date(),
      },
    });
...
export const removeGroupMember = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    await prisma.conversation.update({
      where: { id: conversationId },
      data: { members: { disconnect: { id: userId } }, updatedAt: new Date() },
    });
