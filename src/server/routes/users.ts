...
router.put('/me', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (displayName !== undefined) {
    if (displayName && displayName.trim().length > 50) {
      throw createError('Display name cannot be longer than 50 characters', 400);
    }
...
router.post('/:userId/block', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { userId } = req.params;
...
router.delete('/:userId/block', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { userId } = req.params;
...
// Get user statistics
router.get('/me/stats', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
