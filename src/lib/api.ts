...
class ApiService {  private client: AxiosInstance
  ...
  async disable2FA(password: string, code: string): Promise<ApiResponse> {
    const response = await this.client.post('/auth/disable-2fa', { password, code })
    return response.data
  }
  ...
  async updateProfile(data: { displayName?: string; avatar?: string }): Promise<ApiResponse> {
  ...
  async createDirectConversation(userId: string): Promise<ApiResponse> {
    const response = await this.client.post('/conversations/direct', { userId })
    return response.data
  }

  async createGroupConversation(name: string, userIds: string[]): Promise<ApiResponse> {
    const response = await this.client.post('/conversations/group', { name, userIds })
    return response.data
  }

  async addMembersToGroup(conversationId: string, userIds: string[]): Promise<ApiResponse> {
    const response = await this.client.post(`/conversations/${conversationId}/members`, { userIds })
    return response.data
  }

  async removeMemberFromGroup(conversationId: string, userId: string): Promise<ApiResponse> {
    const response = await this.client.delete(`/conversations/${conversationId}/members/${userId}`)
    return response.data
  }

  async updateGroupInfo(conversationId: string, name: string): Promise<ApiResponse> {
    const response = await this.client.put(`/conversations/${conversationId}`, { name })
    return response.data
  }
  ...
  async getMessages(conversationId: string, page = 1, limit = 50): Promise<ApiResponse> {
    const response = await this.client.get(`/messages/conversation/${conversationId}`, {
      params: { page, limit }
    })
    return response.data
  }
  ...
  async sendMessage(conversationId: string, data: {
    ...
    const response = await this.client.post(`/messages/conversation/${conversationId}`, data)
    return response.data
  }

  async searchMessages(query: string, conversationId?: string, page = 1, limit = 20): Promise<ApiResponse> {
  ...
  async getTypingIndicators(conversationId: string): Promise<ApiResponse> {
    const response = await this.client.get(`/messages/conversation/${conversationId}/typing`)
    return response.data
  }

  // Generic request method
  async request(method: string, url: string, data?: any, params?: any): Promise<ApiResponse> {
