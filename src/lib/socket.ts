...
class SocketService {
  ...
  private messageListeners = new Map<Function, (...args: any[]) => void>() // Store callback mappings
  connect(token: string): void {
    console.log('🔌 Socket.connect called with token:', token ? 'present' : 'missing')
  ...
  onMessageReceived(callback: (message: any) => void): void {
    ...
    // Create wrapper function and store mapping
  ...
  onUserTyping(callback: (data: { userId: string; conversationId: string; username: string }) => void): void {
  ...
  offUserTyping(callback: (data: { userId: string; conversationId: string; username: string }) => void): void {
    this.socket?.off('user_typing', callback)
  }

  offUserStoppedTyping(callback: (data: { userId: string; conversationId: string }) => void): void {
