...
export function useConversations() {  const [conversations, setConversations] = useState<Conversation[]>([])
    ...
    try {
      ...
      if (response.success && response.data?.conversation) {
        ...
        setConversations(prev => {
          ...
          console.log('📝 Updated conversations list:', updated.length, 'total')
          return updated
        })
  ...
  const createGroupConversation = useCallback(async (name: string, userIds: string[]) => {
    try {
      const response = await apiService.createGroupConversation(name, userIds)
      if (response.success && response.data?.conversation) {
        const newConversation = response.data.conversation
  ...
  const updateConversation = useCallback((updatedConversation: Partial<Conversation> & { id: string }) => {
    setConversations(prev =>
      prev.map(conv =>
    ...
    const handleUserTyping = (data: { userId: string; conversationId: string; username: string }) => {
      if (data.conversationId === conversationId) {
        setTypingUsers(prev => [
          ...
          { id: data.userId, userId: data.userId, conversationId: data.conversationId, createdAt: new Date() }
        ])
      }
    }
